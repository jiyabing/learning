...
# 说明：
#    1.handler模块中只有一个ArgvHandler类；
#    2.在main模块中也是实例化了一个ArgvHandler类的对象，并将调用参数传递进去；
#    3.首先，初始化方法会保存调用参数，然后执行parse_args()方法分析参数；
#    4.如果ArgvHandler类有参数指定的功能，则执行该功能，如果没有，打印帮助说明。
#    5.目前ArgvHandler类只有两个核心方法：collect_data和report_dataa；
#    6.这两个方法一个是收集数据并打印到屏幕，用于测试；report_data方法才会将实际的数据发往服务器。
#    7.数据的收集由info_collection.InfoCollection类负责，一会再看；
#    8.report_data方法会将收集到的数据打包到一个字典内，并转换为json格式；
#    9.然后通过settings中的配置，构造发送目的地url；
#   10.通过Python内置的urllib.parse对数据进行封装；
#   11.通过urllib.request将数据发送到目的url；
#   12.接收服务器返回的信息；
#   13.将成功或者失败的信息写入日志文件中。

# 以后，我们要测试数据收集，执行python main.py collect_data；
