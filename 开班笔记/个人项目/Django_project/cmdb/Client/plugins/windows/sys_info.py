...
1.核心在于collect()方法！
2.该方法首先通过platform模块获取平台的信息，然后保存到一个data字典中。
3.然后创建一个Win32Info对象，并调用win32的各种功能方法，分别获取CPU、RAM、主板、硬盘和网卡的信息。
4.类Win32Info是我们编写的封装了具体数据收集逻辑的类；
5.该类中有很多方法，每个方法针对一项数据；
6.其中对Win32模块的调用方式是固定的，有兴趣的可以自行学习这个模块的官方文档
7.每一类的数据收集完成后都会作为一个新的子字典，update到开始的data字典中，最终形成完整的信息字典。
8.最后在脚本末尾有一个测试入口。
"""
